...
async function createRole() {
  ...
  try {
    await storeUtils.createRole(formData.value);
    resetForm();
    modal.handleClickModalRoleAdd()
  } catch (error) {
...
<template>
  <div class="modal" v-if="modal.modalRoleAdd">
    <div class="modal__contenido">
      <div class="form__modal--content">
        <div class="header">
          <button type="button" class="btn__closew" @click="modal.handleClickModalRoleAdd(), resetForm()">
            <img src="/icons/icon_close_line.svg" alt="Close windows" class="btn__icon__closew">
          </button>
        </div>

        <h3 class="form__modal--title">Role Add</h3>
        <hr>
        <form class="form__modal" @submit.prevent="createRole">
