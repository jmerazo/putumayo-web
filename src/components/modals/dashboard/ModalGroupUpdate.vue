...
const initializeFormData = () => {
  const selectedGroup = storeUtils.groupSelected[0];
...
async function updateGroup() {
  ...
  try {
    await storeUtils.createGroup(storeUtils.groupSelected[0].id, formData.value);
    resetForm();
    modal.handleClickModalGroupUpdate()
  } catch (error) {
...
<template>
  <div class="modal" v-if="modal.modalGroupUpdate">
    <div class="modal__contenido">
      <div class="form__modal--content">
        <div class="header">
          <button type="button" class="btn__closew" @click="modal.handleClickModalGroupUpdate(), resetForm()">
            <img src="/icons/icon_close_line.svg" alt="Close windows" class="btn__icon__closew">
          </button>
        </div>

        <h3 class="form__modal--title">Group Add</h3>
        <hr>
        <form class="form__modal" @submit.prevent="updateGroup">
