...
const initializeFormData = () => {
  const selectedSubmodule = storeUtils.submoduleSelected[0];

  if (selectedSubmodule) {
    formData.value = {
      sm_module : selectedSubmodule.sm_module.id || "",
      name: selectedSubmodule.name || "",
      route: selectedSubmodule.route || "",
    };
    iconPreview.value = getFullImageUrl(selectedSubmodule.icon);
  }
};

watch(
  () => storeUtils.submoduleSelected,
  () => {
    initializeFormData();
  }
);
...
async function updateSubmodule() {
  ...
  try {
    ...
    formDataUpload.append('sm_module', formData.value.sm_module);
    ...
    const response = await storeUtils.updateSubmodule(storeUtils.submoduleSelected[0].id, formDataUpload);
    
    if (response.status === 201) {
      resetForm();
      modal.handleClickModalSubmoduleUpdate();
    } else {
      showError("Error update submodule");
    }
  } catch (error) {
    console.error("Error update submodule:", error);
...
<template>
  <div class="modal" v-if="modal.modalSubmoduleUpdate">
    <div class="modal__contenido">
      <div class="form__modal--content">
        <div class="header">
          <button type="button" class="btn__closew" @click="modal.handleClickModalSubmoduleUpdate(), resetForm()">
            <img src="/icons/icon_close_line.svg" alt="Close windows" class="btn__icon__closew">
          </button>
        </div>

        <h3 class="form__modal--title">Submodule Add</h3>
        <hr>
        <form class="form__modal" @submit.prevent="updateSubmodule">

          <div class="form__modal--field">
            <label class="form__modal--label" for="a_rol">Module:</label>
            <select id="d_person" class="form__modal--input" v-model="formData.sm_module">
              <option value="" selected disabled>Select module...</option>
              <option v-for="m in storeUtils.modules" :key="m.id" :value="m.id">
                {{ m.name }}
              </option>
            </select>
          </div>
