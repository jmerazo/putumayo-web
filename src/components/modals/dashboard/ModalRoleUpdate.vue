...
const initializeFormData = () => {
  const selectedRol = storeUtils.rolSelected[0];
...
async function updateRole() {
  ...
  try {
    await storeUtils.rolUpdate(storeUtils.rolSelected[0].id, formData.value);
    resetForm();
    modal.handleClickModalRolUpdate()
  } catch (error) {
...
<template>
  <div class="modal" v-if="modal.modalRolUpdate">
    <div class="modal__contenido">
      <div class="form__modal--content">
        <div class="header">
          <button type="button" class="btn__closew" @click="modal.handleClickModalRolUpdate(), resetForm()">
            <img src="/icons/icon_close_line.svg" alt="Close windows" class="btn__icon__closew">
          </button>
        </div>

        <h3 class="form__modal--title">Role Update</h3>
        <hr>
        <form class="form__modal" @submit.prevent="updateRole">
