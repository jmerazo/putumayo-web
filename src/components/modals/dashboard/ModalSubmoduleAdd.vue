...
async function createSubmodule() {
  ...
  try {
    ...
    const response = await storeUtils.createSubmodule(formDataUpload);
    
    if (response.status === 201) {
      resetForm();
      modal.handleClickModalSubmoduleAdd();
    } else {
      showError("Error creating submodule");
    }
  } catch (error) {
...
<template>
  <div class="modal" v-if="modal.modalSubmoduleAdd">
    <div class="modal__contenido">
      <div class="form__modal--content">
        <div class="header">
          <button type="button" class="btn__closew" @click="modal.handleClickModalSubmoduleAdd(), resetForm()">
        ...
        <hr>
        <form class="form__modal" @submit.prevent="createSubmodule">
