...
<template>
  <div class="users-list">
    <div class="title__users">
        <h3>Subdependencies Putumayo</h3>
    </div>
    <div class="btn__panelUsers">
      <button @click.prevent="modal.handleClickModalSubdependencieAdd()" type="button" class="btn__adduser">
        <img src="/icons/icon_add.svg" class="icon__panel">
      </button>
    </div>    
    <hr class="seccion__divisor">
    <div class="cards-container">
      <div v-for="s in utilsStore.subdependencies" :key="s.id" class="card">
        <div class="icon__subdependencies">
          <img src="/icons/icon_subdependencies.svg">
        </div>
        <div class="card-header">
          <h2>{{ s.name }}</h2>
          <p class="txt__email">{{ s.dependencie.name }}</p>
